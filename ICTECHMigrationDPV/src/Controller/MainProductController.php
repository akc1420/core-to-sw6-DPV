...
namespace ICTECHMigrationDPV\Controller;
...
class MainProductController extends AbstractController
{
    ...
    public function mainProduct(Context $context): Response
    {
        ...
        $configBaseUrlForImage = $this->systemConfigService
            ->get('ICTECHMigrationDPV.config.baseUrlForUploadImage');
        ...
        $servername = $this->systemConfigService
            ->get('ICTECHMigrationDPV.config.databaseHost');
        $username = $this->systemConfigService
            ->get('ICTECHMigrationDPV.config.databaseUser');
        $password = $this->systemConfigService
            ->get('ICTECHMigrationDPV.config.databasePassword');
        $database = $this->systemConfigService
            ->get('ICTECHMigrationDPV.config.databaseName');

        $conn = new mysqli($servername, $username, $password, $database);

        $totalProduct = 0;
        $offSet = $this->systemConfigService->get('ICTECHMigrationDPV.config.mainProductCount');
        ...
        if (mysqli_num_rows($productDetails) > 0) {
            while ($row = mysqli_fetch_assoc($productDetails)) {
                ...
                $this->systemConfigService
                    ->set('ICTECHMigrationDPV.config.mainProductCount', $currentCount);
            }
        }
        if ($offSet < $totalProduct) {
            ...
            $this->systemConfigService
                ->set('ICTECHMigrationDPV.config.mainProductCount', 0);
            $responseArray['type'] = 'Success';
            $responseArray['importProduct'] = $offSet + 1;
            ...
            while ($product = mysqli_fetch_assoc($productDataDetails)) {
                ...
                foreach ($languageDetails as $_language) {
                    ...
                    if ($product['language'] === $languageArray[0]) {
                        ...
                        // Custom data for product_data_buecher

                        $productBuecherDataSql = 'SELECT * from product_data_buecher where product_id = ' .$product['product_id'];
                        $productBuecherDataDetails = mysqli_query($conn, $productBuecherDataSql);

                        if (mysqli_num_rows($productBuecherDataDetails) > 0) {
                            while ($field = mysqli_fetch_assoc($productBuecherDataDetails)) {
                                $customFieldsData['custom_product_buecher_Seitenzahl'] = $field['Seitenzahl'];
                                $customFieldsData['custom_product_buecher_Erscheinungsjahr'] = $field['Erscheinungsjahr'];
                                $customFieldsData['custom_product_buecher_Auflagenart'] = $field['Auflagenart'];
                                $customFieldsData['custom_product_buecher_Reihe'] = $field['Reihe'];
                                $customFieldsData['custom_product_buecher_Produktform'] = $field['Produktform'];
                                $customFieldsData['custom_product_buecher_slogan'] = $field['slogan'];
                                $customFieldsData['custom_product_buecher_notations'] = $field['notations'];
                            }
                        }

                        // Custom data for product_data_ebooks

                        $productBuecherDataSql = 'SELECT * from product_data_ebooks where product_id = ' .$product['product_id'];
                        $productBuecherDataDetails = mysqli_query($conn, $productBuecherDataSql);

                        if (mysqli_num_rows($productBuecherDataDetails) > 0) {
                            while ($field = mysqli_fetch_assoc($productBuecherDataDetails)) {
                                $customFieldsData['custom_product_ebooks_issn'] = $field['issn'];
                                $customFieldsData['custom_product_ebooks_fileformat'] = $field['fileformat'];
                                $customFieldsData['custom_product_ebooks_filesize'] = $field['filesize'];
                                $customFieldsData['custom_product_ebooks_Seitenzahl'] = $field['Seitenzahl'];
                                $customFieldsData['custom_product_ebooks_Erscheinungsjahr'] = $field['Erscheinungsjahr'];
                                $customFieldsData['custom_product_ebooks_Auflagenart'] = $field['Auflagenart'];
                                $customFieldsData['custom_product_ebooks_Reihe'] = $field['Reihe'];
                                $customFieldsData['custom_product_ebooks_Produktform'] = $field['Produktform'];
                                $customFieldsData['custom_product_ebooks_slogan'] = $field['slogan'];
                                $customFieldsData['custom_product_ebooks_notations'] = $field['notations'];
                            }
                        }

                        // Custom data for product_data_sonstiges
                        $productBuecherDataSql = 'SELECT * from product_data_sonstiges where product_id = ' .$product['product_id'];
                        $productBuecherDataDetails = mysqli_query($conn, $productBuecherDataSql);

                        if (mysqli_num_rows($productBuecherDataDetails) > 0) {
                            while ($field = mysqli_fetch_assoc($productBuecherDataDetails)) {
                                $customFieldsData['custom_product_sonstiges_Reihe'] = $field['Reihe'];
                                $customFieldsData['custom_product_sonstiges_Produktform'] = $field['Produktform'];
                                $customFieldsData['custom_product_sonstiges_slogan'] = $field['slogan'];
                                $customFieldsData['custom_product_sonstiges_notations'] = $field['notations'];
                            }
                        }

                        // Custom data for product_data_zeitschriften

                        $productBuecherDataSql = 'SELECT * from product_data_zeitschriften where product_id = ' .$product['product_id'];
                        $productBuecherDataDetails = mysqli_query($conn, $productBuecherDataSql);

                        if (mysqli_num_rows($productBuecherDataDetails) > 0) {
                            while ($field = mysqli_fetch_assoc($productBuecherDataDetails)) {
                                $customFieldsData['custom_product_zeitschriften_issn'] = $field['issn'];
                                $customFieldsData['custom_product_zeitschriften_Seitenzahl'] = $field['Seitenzahl'];
                                $customFieldsData['custom_product_zeitschriften_Erscheinungsjahr'] = $field['Erscheinungsjahr'];
                                $customFieldsData['custom_product_zeitschriften_Auflagenart'] = $field['Auflagenart'];
                                $customFieldsData['custom_product_zeitschriften_Reihe'] = $field['Reihe'];
                                $customFieldsData['custom_product_zeitschriften_Produktform'] = $field['Produktform'];
                                $customFieldsData['custom_product_zeitschriften_slogan'] = $field['slogan'];
                                $customFieldsData['custom_product_zeitschriften_notations'] = $field['notations'];
                            }
                        }
...
//            $galleryMedia = $this->getGalleryMedia($context, $row['p_id'], $conn);
            ...
            $productArray['media'] = $productArray['media'];
            $i = 0;
            if (isset($productArray['media'])) {
...
//        dd($coverImageUrl);
    ...
    public function addImageToMedia(Context $context, array $row): ?string
    {
        ...
        if (!@file_get_contents($imageUrl)) {
            $imageUrl = $this->baseURL . '/' . $row['product_id'] . '/middle/' . $row['image'];
        }
    ...
    private function getPrices($row, Context $context): array
    {
        ...
        $price_array = [];
            ...
            if($row['vat_key'] == 1){
                $netPrice = $row['price'] / 19 ;
            }else if($row['vat_key'] == 2){
                $netPrice = $row['price'] / 7 ;
            }else{
                $netPrice = $row['price'];
            }
            ...
            $price_array = [
                'currencyId' => $currencyId,
                'gross' => $row['price'] === null ? 0 : $row['price'],
                'net' => $netPrice,
                'linked' => true,
            ];
