 ...
 */
class VariantProductPartsController extends AbstractController
{
    ...
    /**
     * @Route("/api/_action/migration/addvariantproductpart",name="api.custom.migration.addvariantproductpart", methods={"POST"})
     */
    public function variantProductPart(Context $context): Response
    {
        ...
        $totalProduct = 0;
        $offSet = $this->systemConfigService->get('ICTECHMigrationDPV.config.variantProductPartCount');

        $productCountSql = 'SELECT COUNT(*) as total_products FROM product_parts';
        ...
        $productSql2 = 'SELECT * FROM product_parts  LIMIT 1 OFFSET ' . $offSet;
        $variantProductDetails2 = mysqli_query($conn, $productSql2);

        if (mysqli_num_rows($variantProductDetails2) > 0) {
            while ($row = mysqli_fetch_assoc($variantProductDetails2)) {
                $this->variantProductUpsert2($row, $context, $conn);
                $currentCount = $offSet + 1;
                $this->systemConfigService
                    ->set('ICTECHMigrationDPV.config.variantProductPartCount', $currentCount);
            }
        }
        ...
        if ($offSet < $totalProduct) {
            ...
            $this->systemConfigService
                ->set('ICTECHMigrationDPV.config.variantProductPartCount', 0);
            $responseArray['type'] = 'Success';
            $responseArray['importVariant'] = $offSet + 1;
    ...
    public function variantProductUpsert2(array $row, Context $context, $conn): void
    {
        ...
        $productDataSql = 'SELECT * from product_data where product_id = ' . $row['product_id'];
        $productDataDetails = mysqli_query($conn, $productDataSql);
        $media_array = [];
        $variantArray = [];

        $parentData = $this->getProductParentId($context, $row['product_id']);
        if($parentData) {
            ...
            while ($product = mysqli_fetch_assoc($productDataDetails)) {
                foreach ($languageDetails as $_language) {
                    ...
                    if ($product['language'] === $languageArray[0]) {
                        ...
                        $productPartsDataSql = 'SELECT * from product_parts_data where parts_id = ' . $row['pp_id'];
                        $dataQuery = mysqli_query($conn, $productPartsDataSql);
                        $data = mysqli_fetch_assoc($dataQuery);

                        if ($data['title'] !== '' && $data['title'] !== null) {
                            $productArray['name'][$languageCode] = $data['title'];
                        } else {
                        ...
                        $customFieldsData['custom_product_part_data_id'] = $row['pp_id'];
                        ...
                        $variantArray['name']['en-GB'] = $product['subtitle'] === null ? '' : $product['subtitle'];
...
//            }
            ...
            $productStockDataSql = 'SELECT * from product where p_id = ' . $row['product_id'];
            $productDataDStocketails = mysqli_query($conn, $productStockDataSql);

            $productStock = mysqli_fetch_assoc($productDataDStocketails);

            $productArray['price'] = $this->getPrices2($row, $context);
            $productArray['stock'] = (int)$productStock['quantity_available'];
    ...
    ): ?Entity
    {
        ...
        $criteria->addFilter(
            new EqualsFilter(
                'customFields.custom_product_part_data_id',
                $productId
            )
        );
    ...
    public function checkProductExistsInProductTable2(
        Context $context,
        string  $productId
    ): ?Entity
    {
    ...
    private function getPrices2($row, Context $context): array
    {
        $servername = $this->systemConfigService
        ->get('ICTECHMigrationDPV.config.databaseHost');
    $username = $this->systemConfigService
    ...
    $conn = new mysqli($servername, $username, $password, $database);
        ...
        $productPartsDataSql = 'SELECT * from product where p_id = ' . $row['product_id'];
        $dataQuery = mysqli_query($conn, $productPartsDataSql);
        $data = mysqli_fetch_assoc($dataQuery);

        if($data['vat_key'] == 1){
            $netPrice = $row['price'] / 19 ;
        }else if($data['vat_key'] == 2){
            $netPrice = $row['price'] / 7 ;
        }else{
            $netPrice = $row['price'];
        }
